...
- Add a Homebrew formula template suitable for `marlonj/tap`.
...
- Do not publish an external Homebrew tap from this workspace.
...
- 2026-07-01: Document `marlonj/tap` as the intended tap name, with the formula stored in this repo for copying into `homebrew-tap`.
...
- 2026-07-01: Added `packaging/homebrew/Formula/ginsights.rb`, a source-build formula suitable for `marlonj/tap`.
...
1. Copy `packaging/homebrew/Formula/ginsights.rb` into `marlonj/homebrew-tap` as `Formula/ginsights.rb` when publishing the tap.
